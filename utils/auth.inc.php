...
class Auth {
  ...
  public function compareUserBookingDate($user, $year, $month, $day, $time){
    if($user["booking"] === "") return FALSE;
    else{
        $bookingData = explode(' ', $user["booking"]);
        $bookingDate = explode('.', $bookingData[0]);
        $bookingTime = $bookingData[1];
        return $year == $bookingDate[0] && $month == $bookingDate[1] && $day == $bookingDate[2] && $time === $bookingTime;
    }
  }
}